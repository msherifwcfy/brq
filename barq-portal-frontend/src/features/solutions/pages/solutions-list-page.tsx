import DashboardLayout from "@/shared/components/layout/DashboardLayout";
import { Button } from "@/shared/components/ui/button";
import { DataTable } from "@/shared/components/ui/data-table";
import { PlusIcon } from "lucide-react";
import React from "react";
import { useSolutionColumns } from "@/features/solutions/columns/solutions-columns";
import { PERMISSION_KEYS } from "@/shared/config/permissions";
import { ProtectedComponent } from "@/shared/components/rbac/ProtectedComponent";
import { useSolutionsControllerReadQuery } from "@/sdk/modules/solution.gen";
import { useUpdateQueryParam } from "@/shared/hooks/useUpdateQueryParam";
import { useModal } from "@/shared/store/modal-store";
import { toSdkFilters } from "@/shared/lib/utils";
import { useLang } from "@/shared/hooks/use-lang";

const SolutionListPage: React.FC = () => {
  const { t } = useLang();
  const { onOpen } = useModal();

  const { getQueryParam } = useUpdateQueryParam("solutions");

  const page = Number(getQueryParam("page")) || 0;
  const limit = Number(getQueryParam("limit")) || 10;
  const keyword = getQueryParam("keyword") || "";
  const createdAtFilter = getQueryParam("created_at") || "";
  const updatedAtFilter = getQueryParam("updated_at") || "";
  const sortBy = getQueryParam("sort_by") || "";
  const sortOrder = getQueryParam("sort_order") || "";

  const {
    data: solutionsData,
    isLoading,
    refetch,
  } = useSolutionsControllerReadQuery({
    query: {
      query: {
        filters: toSdkFilters(
          {
            name: keyword,
            created_at: createdAtFilter,
            updated_at: updatedAtFilter,
          },
          {
            name: "Contains",
            created_at: "Eq",
            updated_at: "Eq",
          }
        ),
        pagination: {
          skip: page * limit,
          take: limit,
        },
        orders: {
          [sortBy]: sortOrder as "asc" | "desc",
        },
        relations: {
          solutions_id_solutions_translations: true,
        },
      },
    },
  });

  const solutions = solutionsData?.data || [];
  const total = solutionsData?.meta?.total || 0;
  const pagesCount = Math.ceil(total / limit);

  const handleCreate = () => {
    onOpen("createSolution", {}, refetch);
  };

  const handleEdit = (solution: any) => {
    onOpen("updateSolution", { solution }, refetch);
  };

  const handleDelete = (solution: any) => {
    onOpen("deleteSolution", { solution }, refetch);
  };

  const solutionColumns = useSolutionColumns(handleEdit, handleDelete, t);

  return (
    <DashboardLayout>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">{t("solutions.title")}</h1>
        <ProtectedComponent permissionKey={PERMISSION_KEYS.SOLUTIONS.CREATE}>
          <Button onClick={handleCreate}>
            <PlusIcon className="w-4 h-4" />
            {t("solutions.createSolution")}
          </Button>
        </ProtectedComponent>
      </div>

      <DataTable
        columns={solutionColumns}
        data={solutions}
        loading={isLoading}
        page={page}
        limit={limit}
        pagesCount={pagesCount}
        tableId="solutions"
        filters={true}
        filtersConfig={{
          fields: [
            {
              key: "created_at",
              label: t("solutions.filters.createdAt"),
              type: "date",
            },
            {
              key: "updated_at",
              label: t("solutions.filters.updatedAt"),
              type: "date",
            },
          ],
        }}
        sortConfig={{
          fields: [
            { key: "name", label: t("solutions.sort.name") },
            { key: "created_at", label: t("solutions.sort.createdAt") },
          ],
        }}
      />
    </DashboardLayout>
  );
};

export default SolutionListPage;
