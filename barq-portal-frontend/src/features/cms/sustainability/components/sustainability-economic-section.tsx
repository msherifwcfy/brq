...
import {
  useEconomicSustainabilityControllerReadQuery,
  useEconomicSustainabilityControllerCreate,
  useEconomicSustainabilityControllerUpdate,
} from "@/sdk/modules/economicsustainability.gen";
...
import {
  createEconomicSustainabilitySchema,
  type CreateEconomicSustainabilityFormData,
} from "../schemas/sustainability-economic.schema";

type FormData = CreateEconomicSustainabilityFormData;

export default function SustainabilityEconomicSection() {
  const { lang, t } = useLang();
  const [currentLanguage, setCurrentLanguage] = useState<LanguageCode>("en");

  const { data } = useEconomicSustainabilityControllerReadQuery({
    query: {
      query: {
        relations: {
          media: true,
          economic_sustainability_id_economic_sustainability_translations: true,
        },
  ...
  const createMutation = useEconomicSustainabilityControllerCreate();
  const updateMutation = useEconomicSustainabilityControllerUpdate();

  const existing = data?.data?.[0];
  const currentTranslation = useMemo(
    () =>
      existing?.economic_sustainability_id_economic_sustainability_translations?.find(
  ...
  const form = useForm<FormData>({
    resolver: zodResolver(createEconomicSustainabilitySchema),
    defaultValues: {
      title: { en: "", ar: "" },
      description: { en: "", ar: "" },
      media: mediaValue,
